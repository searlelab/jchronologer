...
/**
 * Converts 116-bin Scout outputs into filtered ion arrays.
 */
public final class ScoutSpectrumDecoder {

    public static final int VECTOR_LENGTH = 116;
    private static final int MAX_FRAGMENT_ION_NUMBER = 29;

    private ScoutSpectrumDecoder() {
    }
    ...
    public static DecodedSpectrum decode(
        ...
        if (intensities.length != VECTOR_LENGTH) {
            throw new IllegalArgumentException(
                    "Expected Scout vector length " + VECTOR_LENGTH + " but received " + intensities.length);
        }
